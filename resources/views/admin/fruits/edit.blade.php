<x-layout.app>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <x-admin.form :action="route('admin.fruits.update', $fruit)" method="PUT" enctype="multipart/form-data">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-input-label for="name" :value="__('Name')" />
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $fruit->name)" required autofocus />
                    ...
                    <div>
                        ...
                        <x-text-input id="taste_profile" name="taste_profile" type="text" class="mt-1 block w-full" :value="old('taste_profile', $fruit->taste_profile)" />
                        <x-input-error :messages="$errors->get('taste_profile')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="seasonality" :value="__('Seasonality')" />
                        <x-text-input id="seasonality" name="seasonality" type="text" class="mt-1 block w-full" :value="old('seasonality', $fruit->seasonality)" />
                        <x-input-error :messages="$errors->get('seasonality')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="price" :value="__('Price')" />
                        <x-text-input id="price" name="price" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('price', $fruit->price)" />
                    ...
                    <div>
                        ...
                        @if($fruit->image)
                            <div class="mt-2 mb-4">
                                ...
                                <img src="{{ $fruit->image }}" alt="{{ $fruit->name }}" class="h-32 w-32 object-cover rounded-lg">
                            </div>
                        @endif
                    ...
                    <div class="flex items-center space-x-6">
                        <div class="flex items-center">
                            <input id="is_available" name="is_available" type="checkbox" class="h-4 w-4 text-green-800 focus:ring-green-700 border-gray-300 rounded" {{ old('is_available', $fruit->is_available) ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center">
                            <input id="is_featured" name="is_featured" type="checkbox" class="h-4 w-4 text-yellow-500 focus:ring-yellow-400 border-gray-300 rounded" {{ old('is_featured', $fruit->is_featured) ? 'checked' : '' }}>
