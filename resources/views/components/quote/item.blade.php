        ...
        <div class="p-4 sm:w-3/4">
            ...
            <div class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                @if($item->fruit->origin)
                    <p><span class="font-medium">Origin:</span> {{ $item->fruit->origin }}</p>
                @endif
                
                @if($item->fruit->seasonality)
                    <p><span class="font-medium">Seasonality:</span> {{ $item->fruit->seasonality }}</p>
                @endif
            </div>
            
            <div class="mt-4">
                <a href="{{ route('fruits.show', $item->fruit) }}" class="text-green-800 dark:text-yellow-400 hover:underline text-sm">
                    View fruit details
                </a>
            </div>
        </div>
